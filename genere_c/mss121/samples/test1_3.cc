...
int main()
{
   MSS_LOG_MSG("DECLARATIONS ZONE"); //To mark the decl. zone in the logfile
   String str1;
   String str2("Hello Underworld!");
   String str3("Hello Underworld!");


   MSS_LOG_MSG("'IF' ZONE"); //To mark the 'if' zone in the log file
   if(str2.is_equal_to(str3))
      cout<<"equal"<<endl;
   else
      cout<<"different"<<endl;

   MSS_LOG_MSG("'LENGTH' ZONE."); //To mark the 'length' zone in the log file
   cout<<"String length is: "<<str3.Length()<<endl;

   MSS_LOG_MSG("PROGRAM END ZONE"); //To mark the end of the code in the logfile
}
